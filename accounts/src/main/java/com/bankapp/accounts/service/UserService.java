...
import com.bankapp.accounts.feign.exchange.ExchangeFeignClient;
...
public class UserService {
        ...
        for (Account userAccount : userAccounts) {
            if (!accountSettingsDto.getAccounts().contains(userAccount.getCurrency())) {
                if (userAccount.getBalance() != 0) {
                    response.getErrors().add("Чтобы закрыть счет " + userAccount.getCurrency()
                            + " нужно чтобы баланс был равен 0.");
                } else {
    ...
    public ResponseDto processCash(String login, CashDto cashDto) {
        ...
        if ("GET".equals(cashDto.getAction())) {
            notificationsOutboxService.createNotification(login, "Снятие наличных: "
                    + cashDto.getValue() + " " + cashDto.getCurrency());
            accountToModify.setBalance(accountToModify.getBalance() - cashDto.getValue() * 100);
        }
        if ("PUT".equals(cashDto.getAction())) {
            notificationsOutboxService.createNotification(login, "Зачисление наличных: "
                    + cashDto.getValue() + " " + cashDto.getCurrency());
            accountToModify.setBalance(accountToModify.getBalance() + cashDto.getValue() * 100);
        }
        userRepository.save(user);
    ...
    public ResponseDto processTransfer(String login, TransferDto transferDto) {
        ...
        if (!login.equals(transferDto.getToLogin())) {
            notificationsOutboxService.createNotification(login, "Перевод " + transferDto.getValue()
                    + " " + transferDto.getFromCurrency() + " -> " + transferDto.getToCurrency()
                    + " (пользователю " + transferDto.getToLogin() + ")");
            notificationsOutboxService.createNotification(transferDto.getToLogin(), "Зачисление "
                    + transferDto.getValue() + " " + transferDto.getFromCurrency() + " -> "
                    + transferDto.getToCurrency() + " (от пользователя " + login + ")");
            userRepository.save(toUser);
        } else {
            notificationsOutboxService.createNotification(login, "Перевод " + transferDto.getValue()
                    + " " + transferDto.getFromCurrency() + " -> " + transferDto.getToCurrency() + " (на свой счет)");
        }
        return response;
    }
